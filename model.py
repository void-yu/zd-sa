...
class EncoderModel(object):
    ...
    def embedding_layer(self, inputs):
        ...
        embeded_outputs = tf.nn.dropout(embeded_outputs, keep_prob=0.5)
    ...
    def bi_attn_lstm_layer(self, inputs, lenth):
        drop_lstm_fw_cell = tf.contrib.rnn.DropoutWrapper(cell=self.lstm_fw_cell, output_keep_prob=0.5)
        drop_lstm_bw_cell = tf.contrib.rnn.DropoutWrapper(cell=self.lstm_bw_cell, output_keep_prob=0.5)
    ...
    def bi_sigmoid_layer(self, inputs):
        self.sigmoid_weights = tf.Variable(tf.random_uniform(shape=[self.hidden_size*2, 1],
                                                        minval=-1.0, maxval=1.0), name='softmax_weights')
        self.sigmoid_biases = tf.Variable(tf.zeros([1]), name='softmax_biases')
    ...
    def build_validate_graph(self, dev_batch_size):
        # IO
        self.dev_inputs = tf.placeholder(tf.int32, shape=[dev_batch_size, self.seq_size])
        ...
        with tf.name_scope('attention'), tf.variable_scope('attention'):
            ...
            outputs = tf.reshape(rnn_outputs, [dev_batch_size, self.seq_size, 2 * self.hidden_size])
            attn_z = tf.reshape(tf.matmul(attn_z, self.u2_w), [dev_batch_size, self.seq_size])
            alpha = tf.nn.softmax(attn_z)
            alpha = tf.reshape(alpha, [dev_batch_size, self.seq_size, 1])
            rnn_outputs = tf.reduce_sum(outputs * alpha, axis=1)

        # sigmoid
        logits = tf.matmul(rnn_outputs, self.sigmoid_weights) + self.sigmoid_biases
